...
import logging
import json
from data_handler import add_increment_userDB, add_wordsDB, get_wordsDB, check_listDB, check_userSessionsDB, get_paidDB, set_paidDB, set_gradeDB, get_GradewordsDB
...
def create_word_attributes(correctWord, wordSpelled, wordsLeftList, nuCorrect, nuTotal):
    return {"correctWord": correctWord, "wordSpelled": wordSpelled, "wordsLeft": wordsLeftList, "nuCorrect": nuCorrect, "nuTotal": nuTotal}
...
def welcome_text(sessionCount, listCount):
    if sessionCount == 1 and len(listCount) == 0:
        ...
        speech_output = """Welcome back to the Spelling Samurai skill. Please say start quiz <break/> or <break/> add words. Remember <break/> if you need help, say <break/>I need help."""
...
def get_help_response():
    ...
    First, you must create a spelling words list by saying <break/> add words. Then, say your words one after another in succession. Once we have a list, you can say <break/> quiz me. 
    You can also have multiple lists. If you want to add words to a different list you would say <break/> add words to list two.
    Then, to be tested on that list, you would say <break/> quiz me on list two.
    At the end of the test, I tell you your score.<break/> 
    ...
    reprompt_text = "So please tell me what you'd like to do: add words <break/>or<break/> start quiz."
...
def listNotExist():
    ...
    speech_output = "That list doesn't exist. "
    speech_output += "Please tell me to quiz you on a list that does exist."
...
def add_words(session, intent_request={'dialogState':'In_Progress'}):
    ...
    if dialog_state != "COMPLETED":
        ...
        if get_paidDB(session) == 1:
            ...
            reprompt_text = "If you'd like to add more words, just say add more words, otherwise <break/> say start quiz."
            speech_output += reprompt_text
        else: 
            
            
            #if user not paid, check nu of words in list
            if len(get_wordsDB(session, listChoice)) + len(wordList) <= maxWordsNumber:
                add_wordsDB(session, wordList, listNum = listChoice)
                speech_output += "Words added to list %s. " %(listChoice)
                speech_output += "Those words are: %s. " %(wordString)
                reprompt_text = "If you'd like to add more words, just say add more words, otherwise <break/> say start quiz."
                ...
                #the number of words exceeds the max
                ...
                if len(wordList) > 0:
                    ...
                    speech_output += "Words added to list %s. " %(listChoice)
                    speech_output += "Those words are: %s. " %(wordString)
                    reprompt_text = "I can only add a total of %s words <break/>. To support the developer, please consider purchasing the full version of the skill, in which there is no limit. <break/> If you'd like more details before you buy <break/> please say <break/> purchase unlimited words."%(maxWordsWord)
                    speech_output += reprompt_text
                else:
                    speech_output += "You already have %s words in this list <break/> <break/>"%(maxWordsWord)
                    reprompt_text = "To support the developer, please consider purchasing the full version of this skill <break/>, in which there is no limit on word count. If you'd like more details, <break/> please say <break/> purchase unlimited words."
                speech_output += reprompt_text
...
def spelling_quiz(intent, session, startVar):
    ...
    if startVar == 1:
        ...
        session_attributes = create_word_attributes(correctWord, "", wordList, 0, nuTotal)
        ...
        speech_output = letsstartS + "How do you spell " + correctWord + "?<break time = '1s'/><audio src='soundbank://soundlibrary/ui/gameshow/amzn_ui_sfx_gameshow_neutral_response_01'/>"
        ...
        if len(wordList) > 0:
            ...
            speech_output += "<break time='1s'/>" + newS + howS + correctWord + "?<break time = '1s'/><audio src='soundbank://soundlibrary/ui/gameshow/amzn_ui_sfx_gameshow_neutral_response_01'/>"
            session_attributes = create_word_attributes(correctWord, "", wordList, nuCorrect, nuTotal)
        else:
            if nuCorrect > 1:
                ps = 'words'
            else:
            ...
            speech_output += '<break/>Did you enjoy the Spelling Samurai skill?<break/> Please take a moment to rate the skill in the skill store, <break/> or email us for ways to better the experience. <break/> Thank you.'
            should_end_session = True
            session_attributes = create_word_attributes("", "", [], 0, 0)
...
def on_intent(intent_request, session):
    ...
    if intent_name == "StartTestIntent":
        return spelling_quiz(intent, session, 1)
    elif intent_name == "SpellingIntent":
        return spelling_quiz(intent, session, 0)
